...
export function buildLetter(result, comparison, details) {
  ...
  lines.push("");
  lines.push("The numbers I used, taken from the statement's projection for the next 12 months:");
  lines.push("- Escrow balance at the start of the next 12 months: " + formatCents(inputs.startingBalanceCents));
  lines.push("- First of those 12 months: " + monthName(inputs.startMonth));
  lines.push("- Expected bills:");
  for (const line of billLines(result)) lines.push(line);
  lines.push("  Total for the 12 months: " + formatCents(result.annualDisbursementsCents));
  lines.push("");
  lines.push("What the method in 12 CFR 1024.17(d)(2) gives from those numbers:");
  lines.push("- Monthly escrow payment (total ÷ 12): " + formatCents(result.baseMonthlyPaymentCents));
  lines.push("- Most cushion allowed (" + cushionMonthsWords + " of escrow payments): " + formatCents(result.cushionCapCents));
